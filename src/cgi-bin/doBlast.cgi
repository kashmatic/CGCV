...
my $location = '/cgi-bin/cgcv/getProfile.cgi?dir='.$createDir.'&dbtype='.$dbtype.'&evalue='.$evalue;
...
<fieldset>
<p class="bodypara">
Please Wait.........<img src="/cgcv/img/rotating_arrow.gif" onload="getOutput()">
<br>
...
$script .= '
	...
	function getOutput(){
		...
		var url="/cgi-bin/cgcv/runBlast.cgi?";
		url += "dbtype='.$dbtype.'";
		url += "&evalue='.$evalue.'";
		url += "&dir='.$createDir.'";
		url += "&email='.$email.'";
...
sub getGenomes {
	...
	## get the taxid of the selected genomes
	my @db  = $cgi->param('s');
	my $listDb;

=comment
	if (scalar(@db) > 1){	
		$listDb = join(' OR taxid = ',@db);
	} else {
		$listDb = $db[0];
	}
	...
	my $accession;
	eval {
		$accession = DB->exec("SELECT accno FROM taxid_accno WHERE taxid = $listDb",);
	};
	displayErrorPage($@) if ($@);
