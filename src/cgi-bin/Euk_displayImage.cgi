...
my $parseFile = $uploadDir.$createdDir."/parseBlastOutput";
...
my $rowstart = $cgi->param("rowstart") || 0;
my $noRows = $cgi->param("noRows") || 5;
...
$tableDisplay .= '
	...
	<tr>
		...
		<form action="Euk_displayImage.cgi" enctype="multipart/form-data" name="displayForm" method="POST">
		Display
...
for ($i = 0; $i < scalar(@genomelist); $i++){
	...
	my $array;
	eval {
		$array = DB->exec("SELECT orgname, seqlength FROM taxid_accno_Euk WHERE accno=?", [$genome]);
	};
	...
	<td colspan="6" style="border-bottom:1px solid black; border-top:1px solid black;">
		<img src="/cgi-bin/Euk_image.cgi?start=1
			&end='.$length.'
			&dir='.$createdDir.'
			&genome='.$genome.'
			&evalue='.$evalue.'
	...
	system("perl Euk_imageCoords.cgi -g $genome -s 1 -e $length -d $createdDir -D $dbtype -E $evalue -I $i");
