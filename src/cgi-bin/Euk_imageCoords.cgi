...
sub print_query {
	...
	for my $k ( keys %genomeInfo) {
		...
		for my $v ( keys %{$genomeInfo{$k}}) {
			...
			my $blastAlnLink = "/cgi-bin/blastAlignment.cgi?query=".$k."&hit=".$genomeInfo{$k}{$v}{"hit"}."&start=".$genomeInfo{$k}{$v}{"qstart"}."&end=".$genomeInfo{$k}{$v}{"qend"}."&dir=".$dir."&dbtype=".$dbtype;
			...
			print MAP '<AREA Shape="rect" Coords="',
				...
				'&dbtype=',
				$dbtype,
				'" onmouseover="return overlib(\'E value: ',$genomeInfo{$k}{$v}{"evalue"}, '<br>Genomic Position: (',$start, ', ', $end,')<br>Click <a href=\\\'javascript:showZoom(', $imgId, ', ', $start, ', ', $end,', ', $genomeLength, ', ', $genomeInfo{$k}{$v}{"evalue"},')\\\'>HERE</a> to zoom into the neighborhood. <br> Click <a href=\\\'',$blastAlnLink,'\\\'>HERE</a> to view the alignment.\', CAPTION, \'', $k, '  \', CENTER, ADAPTIVE_WIDTH,STICKY);" onmouseout="return nd();"',
				'>',
				"\n";
