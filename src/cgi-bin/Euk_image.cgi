...
## To load the genome parse file
my $dbFile = '_____GENOME_DATA_____'.$dbtype.'/'.$genome;
...
my $array;
eval {
	$array = DB->exec("SELECT seqlength FROM taxid_accno_Euk WHERE accno like ?",[$genome]);
};
displayErrorPage("Unable to read taxid accession number") if ($@);
...
eval {
	$array = DB->exec("SELECT DISTINCT gene_id, gene_start, gene_end, strand, description FROM genedetails_Euk WHERE accno like ? AND ( ( gene_start > ? AND gene_end < ? ) OR ( gene_start > ? AND gene_start < ? ) OR ( gene_end > ? AND gene_end < ? ) OR ( gene_start < ? AND gene_end > ? ) ) ORDER BY gene_end", [$genome, $start, $end, $start, $end, $start, $end, $start, $end]);
};
displayErrorPage("Unable to read gene details.") if ($@);
...
foreach (@$array){
	$num++;
	$dataInfo{$num}{@$_[1]}{"geneid"}=@$_[0];
	$dataInfo{$num}{@$_[1]}{"end"}=@$_[2];
	$dataInfo{$num}{@$_[1]}{"strand"}=@$_[3];
	$dataInfo{$num}{@$_[1]}{"desc"}=@$_[4];
}
...
my $white = $image->colorAllocate(225,250,205);
...
sub print_gene {
	...
	for my $k ( sort keys %dataInfo) {
		...
		for my $v ( sort keys %{$dataInfo{$k}}) {
			...
			if ($count < 100){
				my $x = $position+$bar;
				my $seqInfoLink = '/cgi-bin/euGetSequence.cgi?gene='.$k.'&genome='.$genome.'&geneid='.$dataInfo{$k}{$v}{"geneid"}.'&dir='.$dir;
	
				print MAP '<AREA Shape="rect" Coords="',
					$hsend,',',$position,',',
					$heend,',',$x,
					'" Href="/cgi-bin/euGetSequence.cgi?gene=',
					$v,
						'&genome=',
					$genome,		
						'&geneid=',
					...
					'&dir=',
					$dir,
					'" onmouseover="return overlib(\'GI: ',$dataInfo{$k}{$v}{"geneid"},'<br>Genomic Position: (',$dataInfo{$k}{$v}{"start"},', ',$dataInfo{$k}{$v}{"end"},')<br>Click <a href=\\\'',$seqInfoLink,'\\\'>HERE</a> to view Gene details.\', STICKY, CAPTION, \'',$dataInfo{$k}{$v}{"desc"},'   \', CENTER, ADAPTIVE_WIDTH);" onmouseout="return nd();"',
					'>',
					"\n";
			}
...
sub print_query {
	...
	for my $k ( keys %genomeInfo) {
		...
		for my $v ( keys %{$genomeInfo{$k}}) {
			...
			## Check if there are any 1s in the given array.
			...
			my $blastAlnLink = "/cgi-bin/euBlastAlignment.cgi?query=".$k."&hit=".$genomeInfo{$k}{$v}{"hit"}."&start=".$genomeInfo{$k}{$v}{"qstart"}."&end=".$genomeInfo{$k}{$v}{"qend"}."&dir=".$dir."&dbtype=".$dbtype;

			## Write the map information into the file
			print MAP '<AREA Shape="rect" Coords="',
				$hsend,',',$position,',',
				$heend,',',$x,
				'" Href="/cgi-bin/blastAlignment.cgi?query=',
				$k,
				'&hit=',
				$genomeInfo{$k}{$v}{"hit"},
				'&start=',
