...
#        USAGE:  ./INSTALL.pl 
#
#  DESCRIPTION:  Program that downloads and sets up the database for 2
#                Eukaryotic organisms (Homo Sapiens, Mus musculus) from ENSEMBL FTP
#
#      OPTIONS:  ---
# REQUIREMENTS:  ---
#         BUGS:  ---
...
print "Welcome to the Downloader. This will download all the sequences pertaining to selected Eukaryotes from Ensembl\n\n";
...
print "Connected! Logging in with the following credentials.\n\nUsername: $euk{uname}\nPassword: $euk{passwd}\n\n";
...
#---------------------------------------------------------------------------
	...
	foreach my $file(@listing) {
		...
		next if($file =~ /c[0-9]*_\w+/ || $file =~ /dna_rm/ || $file =~ /nonchromo/ || $file =~ /toplevel/ || $file =~ /README/ || $file =~ /Het/ || $file =~ /extra/ || $file =~ /MT/ || $file =~ /mito/ || $file =~ /MtDNA/);
...
#---------------------------------------------------------------------------
	...
	foreach my $file(@files){
		next unless($file =~ /fa$/ || $file =~ /gtf$/ || $file =~ /$eukaryote/);
		print $file, "\n";

		my ($chrno) = $file =~ /^\S+_\S+\.\S+\.\S+\.[\S+\.]*dna\.chromosome\.(\S+)\.fa$/ if($file =~ /dna/);
		$chrno = 1 if($chrno =~ /I/);
		$chrno = 2 if($chrno =~ /II/);
