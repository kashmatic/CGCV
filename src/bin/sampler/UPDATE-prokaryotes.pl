...
foreach my $dir(@directories) {
	...
	my @dir_listing = $ftp->dir;
	my @file_listing = $ftp->ls('*.fna');
...
foreach my $file(@oldFiles) {
    system "diff $bact{dataDir}/listings/$file\.listing $bact{dataDir}/tmpListings/$file\.listing > $file\.diff";
	...
	if( $filesize == 0) {
		...
		system "diff -y -W 150 $bact{dataDir}/listings/$file\.listing $bact{dataDir}/tmpListings/$file\.listing > $file\.diff";
...
foreach my $file(@diffFiles) {
	...
	for my $key(sort keys %timestamp) {
		if( $timestamp{$key}{oldDDMMMYY} ne $timestamp{$key}{newDDMMMYY} ) {
			$check = 1;
			system "printf \"Fetching ftp://$bact{ftpLink}/$bact{directory}/$fname/$key ...\" ; wget ftp://$bact{ftpLink}/$bact{directory}/$fname/$key --quiet --passive-ftp ; echo ...Done!";
			my ($Fn) = $key =~ /^(\S+)\.\S+$/;
...
if( scalar @newFiles > 0 ) {
    ...
    foreach my $dir(@newFiles) {
		if($dir =~ /\//) {
			...
			system "printf \"Fetching ftp://$bact{ftpLink}/$bact{directory}/$dir ...\" ; wget ftp://$bact{ftpLink}/$bact{directory}/$dir --quiet --passive-ftp; echo ...Done!";
			...
			while(<NEWLISTING>) {
				...
				system "printf \"Fetching ftp://$bact{ftpLink}/$bact{directory}/$dir/$file...\" ; wget ftp://$bact{ftpLink}/$bact{directory}/$dir/$file --quiet --passive-ftp; echo ...Done!";
...
if( scalar @deletedFiles > 0 ) {
	...
	foreach my $fname(@del) {
		system "rm $bact{dataDir}/genomes/$fname* $bact{dataDir}/geneseqs/backup/$fname $bact{dataDir}/geneseqs/$fname* $bact{dataDir}/aaseqs/$fname* $bact{dataDir}/gff_files/$fname";
